...
import { mergePantryItems, mergeSeasonings, type IngredientItemDelta, type SeasoningItemDelta } from '../lib/pantryMerge';
...
export function useChat() {
  ...
  const sendUserMessage = useCallback(async (text: string) => {
    ...
    if (!settings.activeProvider || !settings.activeModel) {
      dispatch({
        type: 'ADD_ERROR',
        payload: 'LLM プロバイダーまたはモデルが未選択です。設定画面でプロバイダーとモデルを選んでください。',
      });
      return;
    }
    ...
    try {
      ...
      const isBackside = conversation.recentBacksideActivity || isBacksideTrigger(conversation.lastUserText);
      ...
      // レンダリング前にコード側で必ず落とす。
      {
        ...
        if (import.meta.env.DEV && sanitized.removed) {
          console.warn('[useChat] 本文からメタ漏れを除去しました（モデルが tool_call を本文に書いた可能性）');
        }
      ...
      if (import.meta.env.DEV) {
        const claimsUpdate = /覚え(た|とく|たよ)|更新(した|しとく)|メモ(した|しとく|したよ)|へらしとく|減らしとく|追加(した|しとく)|加えとく|足しとく|フルで覚え/.test(finalContent);
        const calledSaveTool = (res.toolCalls ?? []).some(tc =>
  ...
  }, [isWaiting, turns, isConfigured, settings, state.data, savePantry, appendMealRecord, saveProfile, dispatch, updateCurrentSession]);
