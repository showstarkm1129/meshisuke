      ...
      {localHistory.length === 0 ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0, marginTop: hideTitle ? '0px' : '20px' }}>
          {localHistory.map((record, idx) => (
            <li key={idx} style={{ padding: '12px', borderBottom: '1px solid #ccc', position: 'relative' }}>
              <button 
                onClick={() => deleteRecord(idx)} 
                style={{ position: 'absolute', top: '12px', right: '12px', cursor: 'pointer', background: 'none', border: 'none' }}
                title="削除"
              >🗑️</button>
              
              <strong>{record.date} ({record.meal}): {record.menu}</strong>
              <div style={{ fontSize: '0.9em', color: '#666', marginTop: '4px' }}>
                カロリー: {record.nutrition?.kcal ?? 0}kcal | 満足度: {record.satisfaction ?? '未設定'}
              </div>
