            ...
            >
              ...
              <div style={{ fontSize: '0.9rem', color: '#666', marginBottom: '12px' }}>
                {item.type === 'ingredient' && (
                  <>
                    ...
                    {(item.data as Ingredient).expires_at
                      ? ` | 期限: ${(item.data as Ingredient).expires_at}`
                      : ''}
                  </>
                )}
                {item.type === 'seasoning' &&
                  `残量: ${(item.data as Seasoning).amount_current} / ${(item.data as Seasoning).amount_full} ${(item.data as Seasoning).unit}`}
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {item.type === 'ingredient' ? (
                  <>
                    <button
                      onClick={() => handleAction(item, 'まだある')}
                      style={actionButtonStyle}
                    >
                      まだある
                    </button>
                    <button
                      onClick={() => handleAction(item, '使い切った')}
                      style={actionButtonStyle}
                    >
                      使い切った
                    </button>
                    <button
                      onClick={() => handleAction(item, '期限切れた')}
                      style={actionButtonStyle}
                    >
                      期限切れた
                    </button>
                  ...
                  <>
                    <button
                      onClick={() => handleAction(item, '補充済み')}
                      style={actionButtonStyle}
                    >
                      補充済み
                    </button>
                    <button
                      onClick={() => handleAction(item, 'まだ少ない')}
                      style={actionButtonStyle}
                    >
                      まだ少ない
                    </button>
